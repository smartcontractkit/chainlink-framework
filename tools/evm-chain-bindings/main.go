...
func writeGoCode(contracts map[string]evm.CodeDetails, packageName string, generatedContracts map[string][]byte, err error, goModRelatedOutputDir string, chainReaderConfig types.ChainReaderConfig, chainWriterConfig types.ChainWriterConfig) {
	for contractName, contractDetails := range contracts {
		generatedContractContent, err := evm.GenerateContractBinding(packageName, contractDetails)
		if err != nil {
	...
	configFactoryContent, err := evm.GenerateChainReaderAndWriterFactory(packageName, chainReaderConfig, chainWriterConfig)
	if err != nil {
	...
	utils.GenerateFile(goModRelatedOutputDir, "chain_config_factory", "go", configFactoryContent)
}

func processChainReaderChainWriterConfig(err error, contracts map[string]evm.CodeDetails) (types.ChainReaderConfig, types.ChainWriterConfig) {
	chainReaderConfig, chainWriterConfig, err := evm.GenerateChainReaderChainWriterConfig(contracts)
	if err != nil {
...
func processContracts(contractFolders []string) map[string]evm.CodeDetails {
	contracts := map[string]evm.CodeDetails{}
	for _, contractFolder := range contractFolders {
		files, err := utils.ListFiles(contractFolder, "abi")
		if err != nil {
		...
		for _, file := range files {
			...
			contractName, contractABI, err := utils.LoadFile(file)
			if err != nil {
				debugLogging("Failed loading ABI file  %s", file)
				logError(err, FailedLoadingAbiFile)
			}
			contractDetails, err := evm.ConvertABIToCodeDetails(contractName, contractABI)
...
func failIfNoContractsAndFlagSet(contracts map[string]evm.CodeDetails) {
