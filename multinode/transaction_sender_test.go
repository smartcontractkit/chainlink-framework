...
type TestSendTxRPCClient SendTxRPCClient[any]
...
func (rpc *sendTxRPC) SendTransaction(ctx context.Context, _ any) (SendTxReturnCode, error) {
	if rpc.sendTxRun != nil {
		rpc.sendTxRun(mock.Arguments{ctx})
	}
	return classifySendTxError(nil, rpc.sendTxErr), rpc.sendTxErr
}
...
func newTestTransactionSender(t *testing.T, chainID ID, lggr logger.Logger,
...
) (*sendTxMultiNode, *TransactionSender[any, ID, TestSendTxRPCClient]) {
	...
	txSender := NewTransactionSender[any, ID, TestSendTxRPCClient](lggr, chainID, mn.chainFamily, mn.MultiNode, func(err error) SendTxReturnCode { return 0 }, tests.TestInterval)
...
func TestTransactionSender_SendTransaction(t *testing.T) {
	...
	t.Run("Transaction failure happy path", func(t *testing.T) {
		...
		code, err := txSender.SendTransaction(tests.Context(t), nil)
	...
	t.Run("Context expired before collecting sufficient results", func(t *testing.T) {
		...
		cancel()
		_, err := txSender.SendTransaction(requestContext, nil)
	...
	t.Run("Fails when closed", func(t *testing.T) {
		...
		var txSender *TransactionSender[any, ID, TestSendTxRPCClient]
	...
	t.Run("All background jobs stop even if RPC returns result after soft timeout", func(t *testing.T) {
		...
		_, txSender := newTestTransactionSender(t, chainID, lggr, []Node[ID, TestSendTxRPCClient]{fastNode, slowNode}, nil)
		_, err := txSender.SendTransaction(sendTxContext, nil)
		sendTxCancel()
		require.EqualError(t, err, expectedError.Error())
